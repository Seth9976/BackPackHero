	...
	public class ZipEntry
	{
		...
		private int ExtractOne(Stream output)
		{
			...
			try
			{
				...
				long num2 = ((this._CompressionMethod_FromZipFile == 0) ? this._CompressedFileDataSize : this.UncompressedSize);
				this._inputDecryptorStream = this.GetExtractDecryptor(archiveStream);
				Stream extractDecompressor = this.GetExtractDecompressor(this._inputDecryptorStream);
				long num3 = 0L;
				using (CrcCalculatorStream crcCalculatorStream = new CrcCalculatorStream(extractDecompressor))
				{
					while (num2 > 0L)
					{
						int num4 = ((num2 <= (long)array.Length) ? ((int)num2) : array.Length);
								...
								if (unsupportedAlgorithmId != 0U)
								{
									if (unsupportedAlgorithmId != 26370U)
									{
										if (unsupportedAlgorithmId != 26625U)
										{
											...
											text = string.Format("Unknown (0x{0:X4})", this._UnsupportedAlgorithmId);
										}
										else
										{
											text = "RC4";
					...
					if (compressionMethod != 0)
					{
						if (compressionMethod != 1)
						{
							if (compressionMethod != 19)
							{
								...
								{
									text = string.Format("Unknown (0x{0:X4})", this._CompressionMethod);
								}
								else
								{
									text = "PPMd";
		...
		private long SetInputAndFigureFileLength(ref Stream input)
		{
			...
			{
				string text = ((this._Encryption_FromZipFile != EncryptionAlgorithm.None) ? (this._Password ?? this._container.Password) : null);
