...
namespace Pathfinding.Voxels
{
	...
	{
		// Token: 0x06000780 RID: 1920 RVA: 0x0002E6E8 File Offset: 0x0002C8E8
		public void BuildContours(float maxError, int maxEdgeLength, VoxelContourSet cset, int buildFlags)
		{
				...
				for (int n = 0; n < this.voxelArea.width; n++)
				{
					...
					while (num10 < num11)
					{
						...
						else
						{
							...
							{
								int num12 = this.voxelArea.areaTypes[num10];
								list2.Clear();
								list3.Clear();
								...
								{
									voxelContour.verts[num13] = list3[num13];
								}
								voxelContour.nverts = list3.Count / 4;
								voxelContour.reg = reg;
								voxelContour.area = num12;
			...
			{
				VoxelContour voxelContour2 = list[num14];
				if (this.CalcAreaOfPolygon2D(voxelContour2.verts, voxelContour2.nverts) < 0)
				{
					int num15 = -1;
					for (int num16 = 0; num16 < list.Count; num16++)
					{
						if (num14 != num16 && list[num16].nverts > 0 && list[num16].reg == voxelContour2.reg && this.CalcAreaOfPolygon2D(list[num16].verts, list[num16].nverts) > 0)
						{
							num15 = num16;
							break;
						}
					...
					if (num15 == -1)
					{
						Debug.LogError("rcBuildContours: Could not find merge target for bad contour " + num14.ToString() + ".");
					}
					else
					{
						...
						{
							list[num15] = voxelContour3;
							list[num14] = voxelContour2;
						}
					}
				}
			}
		...
		// Token: 0x06000781 RID: 1921 RVA: 0x0002EC28 File Offset: 0x0002CE28
		private void GetClosestIndices(int[] vertsa, int nvertsa, int[] vertsb, int nvertsb, ref int ia, ref int ib)
		{
			...
			for (int i = 0; i < nvertsa; i++)
			{
				...
				{
					int num7 = j * 4;
					if (Voxelize.Ileft(num6, num4, num7, vertsa, vertsa, vertsb) && Voxelize.Ileft(num4, num5, num7, vertsa, vertsa, vertsb))
					{
						int num8 = vertsb[num7] - vertsa[num4];
						int num9 = vertsb[num7 + 2] / this.voxelArea.width - vertsa[num4 + 2] / this.voxelArea.width;
		...
		// Token: 0x06000782 RID: 1922 RVA: 0x0002ECF0 File Offset: 0x0002CEF0
		...
		// Token: 0x06000783 RID: 1923 RVA: 0x0002ED44 File Offset: 0x0002CF44
		...
		// Token: 0x06000784 RID: 1924 RVA: 0x0002EE74 File Offset: 0x0002D074
		public void SimplifyContour(List<int> verts, List<int> simplified, float maxError, int maxEdgeLenght, int buildFlags)
		{
			...
			int num14 = verts.Count / 4;
			maxError *= maxError;
			int l = 0;
			while (l < simplified.Count / 4)
			{
				int num15 = (l + 1) % (simplified.Count / 4);
				int num16 = simplified[l * 4];
				int num17 = simplified[l * 4 + 2];
				int num18 = simplified[l * 4 + 3];
				int num19 = simplified[num15 * 4];
				int num20 = simplified[num15 * 4 + 2];
				int num21 = simplified[num15 * 4 + 3];
				float num22 = 0f;
				int num23 = -1;
				int num24;
				int num25;
				...
				else
				{
					...
					num26 = num18;
					Memory.Swap<int>(ref num16, ref num19);
					Memory.Swap<int>(ref num17, ref num20);
				}
				if ((verts[num25 * 4 + 3] & 65535) != 0)
				{
					if ((verts[num25 * 4 + 3] & 131072) != 131072)
					{
						goto IL_034F;
					}
				}
				while (num25 != num26)
				{
					float num27 = VectorMath.SqrDistancePointSegmentApproximate(verts[num25 * 4], verts[num25 * 4 + 2] / this.voxelArea.width, num16, num17 / this.voxelArea.width, num19, num20 / this.voxelArea.width);
					if (num27 > num22)
					{
						num22 = num27;
						num23 = num25;
				...
				if (num23 != -1 && num22 > maxError)
				{
					...
					simplified[(l + 1) * 4] = verts[num23 * 4];
					simplified[(l + 1) * 4 + 1] = verts[num23 * 4 + 1];
					simplified[(l + 1) * 4 + 2] = verts[num23 * 4 + 2];
					simplified[(l + 1) * 4 + 3] = num23;
				}
				else
				{
					l++;
			...
			float num28 = this.maxEdgeLength / this.cellSize;
			if (num28 > 0f && (buildFlags & 7) != 0)
			{
				int num29 = 0;
				while (num29 < simplified.Count / 4 && simplified.Count / 4 <= 200)
				{
					int num30 = (num29 + 1) % (simplified.Count / 4);
					int num31 = simplified[num29 * 4];
					int num32 = simplified[num29 * 4 + 2];
					int num33 = simplified[num29 * 4 + 3];
					int num34 = simplified[num30 * 4];
					int num35 = simplified[num30 * 4 + 2];
					int num36 = simplified[num30 * 4 + 3];
					int num37 = -1;
					int num38 = (num33 + 1) % num14;
					bool flag4 = false;
					if ((buildFlags & 1) != 0 && (verts[num38 * 4 + 3] & 65535) == 0)
					{
						flag4 = true;
					}
					if ((buildFlags & 2) != 0 && (verts[num38 * 4 + 3] & 131072) == 131072)
					{
						flag4 = true;
					}
					if ((buildFlags & 4) != 0 && (verts[num38 * 4 + 3] & 32768) == 32768)
					{
						flag4 = true;
					}
					if (flag4)
					{
						...
						if ((float)(num39 * num39 + num40 * num40) > num28 * num28)
						{
					...
					if (num37 != -1)
					{
						...
						for (int n = simplified.Count / 4 - 1; n > num29; n--)
						{
						...
						simplified[(num29 + 1) * 4] = verts[num37 * 4];
						simplified[(num29 + 1) * 4 + 1] = verts[num37 * 4 + 1];
						simplified[(num29 + 1) * 4 + 2] = verts[num37 * 4 + 2];
						simplified[(num29 + 1) * 4 + 3] = num37;
					}
					else
					{
						num29++;
			...
			for (int num42 = 0; num42 < simplified.Count / 4; num42++)
			{
				int num43 = (simplified[num42 * 4 + 3] + 1) % num14;
				int num44 = simplified[num42 * 4 + 3];
				simplified[num42 * 4 + 3] = (verts[num43 * 4 + 3] & 65535) | (verts[num44 * 4 + 3] & 65536);
			}
		}

		// Token: 0x06000785 RID: 1925 RVA: 0x0002F5E0 File Offset: 0x0002D7E0
		...
		// Token: 0x06000786 RID: 1926 RVA: 0x0002F828 File Offset: 0x0002DA28
		...
		// Token: 0x06000787 RID: 1927 RVA: 0x0002FB90 File Offset: 0x0002DD90
		...
		// Token: 0x06000788 RID: 1928 RVA: 0x0002FBF8 File Offset: 0x0002DDF8
		...
		// Token: 0x06000789 RID: 1929 RVA: 0x0002FC57 File Offset: 0x0002DE57
		public static bool Ileft(int a, int b, int c, int[] va, int[] vb, int[] vc)
		{
			return (vb[b] - va[a]) * (vc[c + 2] - va[a + 2]) - (vc[c] - va[a]) * (vb[b + 2] - va[a + 2]) <= 0;
		}

		// Token: 0x0600078A RID: 1930 RVA: 0x0002FC8A File Offset: 0x0002DE8A
		...
		// Token: 0x0600078B RID: 1931 RVA: 0x0002FCA8 File Offset: 0x0002DEA8
		public static bool InCone(int i, int j, int n, int[] verts, int[] indices)
		{
			...
			int num3 = (indices[Voxelize.Next(i, n)] & 268435455) * 4;
			int num4 = (indices[Voxelize.Prev(i, n)] & 268435455) * 4;
			if (Voxelize.LeftOn(num4, num, num3, verts))
			{
				return Voxelize.Left(num, num2, num4, verts) && Voxelize.Left(num2, num, num3, verts);
			}
			return !Voxelize.LeftOn(num, num2, num3, verts) || !Voxelize.LeftOn(num2, num, num4, verts);
		}

		// Token: 0x0600078C RID: 1932 RVA: 0x0002FD30 File Offset: 0x0002DF30
		public static bool Left(int a, int b, int c, int[] verts)
		{
			return Voxelize.Area2(a, b, c, verts) < 0;
		}

		// Token: 0x0600078D RID: 1933 RVA: 0x0002FD3E File Offset: 0x0002DF3E
		public static bool LeftOn(int a, int b, int c, int[] verts)
		{
			return Voxelize.Area2(a, b, c, verts) <= 0;
		}

		// Token: 0x0600078E RID: 1934 RVA: 0x0002FD4F File Offset: 0x0002DF4F
		public static bool Collinear(int a, int b, int c, int[] verts)
		{
			return Voxelize.Area2(a, b, c, verts) == 0;
		}

		// Token: 0x0600078F RID: 1935 RVA: 0x0002FD5D File Offset: 0x0002DF5D
		...
		// Token: 0x06000790 RID: 1936 RVA: 0x0002FD88 File Offset: 0x0002DF88
		private static bool Diagonalie(int i, int j, int n, int[] verts, int[] indices)
		{
			...
			for (int k = 0; k < n; k++)
			{
				int num3 = Voxelize.Next(k, n);
				if (k != i && num3 != i && k != j && num3 != j)
				{
					int num4 = (indices[k] & 268435455) * 4;
					int num5 = (indices[num3] & 268435455) * 4;
					if (!Voxelize.Vequal(num, num4, verts) && !Voxelize.Vequal(num2, num4, verts) && !Voxelize.Vequal(num, num5, verts) && !Voxelize.Vequal(num2, num5, verts) && Voxelize.Intersect(num, num2, num4, num5, verts))
					{
						return false;
					}
				}
		...
		// Token: 0x06000791 RID: 1937 RVA: 0x0002FE2C File Offset: 0x0002E02C
		public static bool Xorb(bool x, bool y)
		{
			return !x ^ !y;
		}

		// Token: 0x06000792 RID: 1938 RVA: 0x0002FE38 File Offset: 0x0002E038
		...
		// Token: 0x06000793 RID: 1939 RVA: 0x0002FEB0 File Offset: 0x0002E0B0
		...
		// Token: 0x06000794 RID: 1940 RVA: 0x0002FF34 File Offset: 0x0002E134
		...
		// Token: 0x06000795 RID: 1941 RVA: 0x0002FF83 File Offset: 0x0002E183
		private static bool Vequal(int a, int b, int[] verts)
		{
			return verts[a] == verts[b] && verts[a + 2] == verts[b + 2];
		}

		// Token: 0x06000796 RID: 1942 RVA: 0x0002FF9B File Offset: 0x0002E19B
		...
		// Token: 0x06000797 RID: 1943 RVA: 0x0002FFAA File Offset: 0x0002E1AA
		...
		// Token: 0x06000798 RID: 1944 RVA: 0x0002FFB8 File Offset: 0x0002E1B8
		...
		// Token: 0x06000799 RID: 1945 RVA: 0x0003021C File Offset: 0x0002E41C
		private int Triangulate(int n, int[] verts, ref int[] indices, ref int[] tris)
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				int num4 = Voxelize.Next(num3, n);
				if (Voxelize.Diagonal(i, num4, n, verts, indices))
				{
			...
			while (n > 3)
			{
				...
				for (int j = 0; j < n; j++)
				{
					int num7 = Voxelize.Next(j, n);
					if ((indices[num7] & 1073741824) != 0)
					{
						int num8 = (indices[j] & 268435455) * 4;
		...
		// (get) Token: 0x0600079A RID: 1946 RVA: 0x00030448 File Offset: 0x0002E648
		// (set) Token: 0x0600079B RID: 1947 RVA: 0x00030450 File Offset: 0x0002E650
		public GraphTransform transformVoxel2Graph { get; private set; }

		// Token: 0x0600079C RID: 1948 RVA: 0x0003045C File Offset: 0x0002E65C
		...
		// Token: 0x0600079D RID: 1949 RVA: 0x000304B8 File Offset: 0x0002E6B8
		...
		// Token: 0x0600079E RID: 1950 RVA: 0x00030508 File Offset: 0x0002E708
		...
		// Token: 0x0600079F RID: 1951 RVA: 0x000305B8 File Offset: 0x0002E7B8
		public void Init()
		{
		...
		// Token: 0x060007A0 RID: 1952 RVA: 0x00030618 File Offset: 0x0002E818
						...
						for (int m = num4; m <= num6; m++)
						{
							...
							if (voxelPolygonClipper2.n >= 3)
							{
								...
								if (voxelPolygonClipper3.n >= 3)
								{
									...
									float num10 = voxelPolygonClipper3.z[0];
									for (int n = 1; n < voxelPolygonClipper3.n; n++)
									{
										float num11 = voxelPolygonClipper3.z[n];
										num9 = Math.Min(num9, num11);
										num10 = Math.Max(num10, num11);
									}
									int num12 = Mathf.Clamp((int)Math.Round((double)num9), 0, this.voxelArea.depth - 1);
									int num13 = Mathf.Clamp((int)Math.Round((double)num10), 0, this.voxelArea.depth - 1);
									for (int num14 = num12; num14 <= num13; num14++)
									{
										voxelPolygonClipper3.ClipPolygonAlongZWithYZ(ref voxelPolygonClipper4, 1f, (float)(-(float)num14) + 0.5f);
										if (voxelPolygonClipper4.n >= 3)
										{
											voxelPolygonClipper4.ClipPolygonAlongZWithY(ref voxelPolygonClipper5, -1f, (float)num14 + 0.5f);
											if (voxelPolygonClipper5.n >= 3)
											{
												float num15 = voxelPolygonClipper5.y[0];
												float num16 = voxelPolygonClipper5.y[0];
												for (int num17 = 1; num17 < voxelPolygonClipper5.n; num17++)
												{
													float num18 = voxelPolygonClipper5.y[num17];
													num15 = Math.Min(num15, num18);
													num16 = Math.Max(num16, num18);
												}
												int num19 = (int)Math.Ceiling((double)num16);
												if (num19 >= 0 && num15 <= (float)num)
												{
													int num20 = Math.Max(0, (int)num15);
													num19 = Math.Max(num20 + 1, num19);
													this.voxelArea.AddLinkedSpan(num14 * this.voxelArea.width + m, (uint)num20, (uint)num19, num8, this.voxelWalkableClimb);
												}
											}
										}
									}
		...
		// Token: 0x060007A1 RID: 1953 RVA: 0x00030BA4 File Offset: 0x0002EDA4
		...
		// Token: 0x060007A2 RID: 1954 RVA: 0x00030D30 File Offset: 0x0002EF30
					...
					else
					{
						...
						while (num6 != -1)
						{
							if (linkedSpans[num6].area != 0)
							{
								...
								int num9 = (int)((next != -1) ? linkedSpans[next].bottom : 65536U);
								this.voxelArea.compactSpans[(int)num] = new CompactVoxelSpan((ushort)((top > 65535) ? 65535 : top), (uint)((num9 - top > 65535) ? 65535 : (num9 - top)));
						...
						this.voxelArea.compactCells[j + i] = new CompactVoxelCell(num7, num8);
					}
				}
				i += num2;
				num5++;
		...
		// Token: 0x060007A3 RID: 1955 RVA: 0x00030F3C File Offset: 0x0002F13C
		...
		// Token: 0x060007A4 RID: 1956 RVA: 0x00031150 File Offset: 0x0002F350
		...
		// Token: 0x060007A5 RID: 1957 RVA: 0x000311A8 File Offset: 0x0002F3A8
		...
		// Token: 0x060007A6 RID: 1958 RVA: 0x000311CC File Offset: 0x0002F3CC
		...
		// Token: 0x060007A7 RID: 1959 RVA: 0x00031245 File Offset: 0x0002F445
		...
		// Token: 0x060007A8 RID: 1960 RVA: 0x00031278 File Offset: 0x0002F478
		...
		// Token: 0x060007A9 RID: 1961 RVA: 0x000312D4 File Offset: 0x0002F4D4
		...
		// Token: 0x060007AA RID: 1962 RVA: 0x00031358 File Offset: 0x0002F558
		...
		// Token: 0x060007AB RID: 1963 RVA: 0x00031408 File Offset: 0x0002F608
		...
		// Token: 0x060007AC RID: 1964 RVA: 0x00031540 File Offset: 0x0002F740
		...
		// Token: 0x060007AD RID: 1965 RVA: 0x00031634 File Offset: 0x0002F834
						...
						for (int num4 = j + i; num4 != -1; num4 = linkedSpans[num4].next)
						{
							if (linkedSpans[num4].area != 0)
							{
								...
								for (int k = 0; k < 4; k++)
								{
									...
									int num13 = (int)((linkedSpans[num11].bottom != uint.MaxValue) ? linkedSpans[num11].bottom : 65536U);
		...
		// Token: 0x060007AE RID: 1966 RVA: 0x000318E4 File Offset: 0x0002FAE4
		...
		// Token: 0x060007AF RID: 1967 RVA: 0x00031BD0 File Offset: 0x0002FDD0
		...
		// Token: 0x060007B0 RID: 1968 RVA: 0x00031C6C File Offset: 0x0002FE6C
		...
		// Token: 0x060007B1 RID: 1969 RVA: 0x00032298 File Offset: 0x00030498
		...
		// Token: 0x060007B2 RID: 1970 RVA: 0x00032498 File Offset: 0x00030698
		public void BuildRegions()
		{
			...
			num5 += 1;
			Int3[][] array6 = new Int3[(int)(this.voxelArea.maxDistance / 2 + 1)][];
				...
				for (int k = 0; k < this.voxelArea.width; k++)
				{
					...
					while (l < num7)
					{
						if ((array[l] & 32768) != 32768 && areaTypes[l] != 0)
						{
							...
							if (array7[num8] >= array6[num8].Length)
							{
								Int3[] array8 = new Int3[array7[num8] * 2];
								array6[num8].CopyTo(array8, 0);
								array6[num8] = array8;
							}
							Int3[] array9 = array6[num8];
							int[] array10 = array7;
							int num9 = num8;
							int num10 = array10[num9];
							array10[num9] = num10 + 1;
							array9[num10] = new Int3(k, l, j);
						}
						l++;
					}
				}
			...
			for (int m = array6.Length - 1; m >= 0; m--)
			{
				uint num11 = (uint)(m * 2);
				Int3[] array11 = array6[m];
				int num12 = array7[m];
				...
				{
					Int3 int2 = array11[num21];
					if (array[int2.y] == 0 && this.FloodRegion(int2.x, int2.z, int2.y, num11, num5, array, array2, array5, array4, array3))
					{
						num5 += 1;
					}
				}
			...
			ArrayPool<int>.Release(ref array4, false);
			ArrayPool<Int3>.Release(ref array5, false);
		}

		// Token: 0x060007B3 RID: 1971 RVA: 0x000329E4 File Offset: 0x00030BE4
		...
		// Token: 0x060007B4 RID: 1972 RVA: 0x00032A08 File Offset: 0x00030C08
		...
		// Token: 0x060007B5 RID: 1973 RVA: 0x00032A44 File Offset: 0x00030C44
