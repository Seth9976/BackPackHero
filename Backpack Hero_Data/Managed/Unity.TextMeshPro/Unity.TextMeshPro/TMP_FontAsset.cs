...
namespace TMPro
{
	...
	public class TMP_FontAsset : TMP_Asset
	{
		...
		internal void AddSynthesizedCharactersAndFaceMetrics()
		{
			...
			{
				flag = FontEngine.LoadFontFace(this.sourceFontFile, this.m_FaceInfo.pointSize) == FontEngineError.Success;
			}
		...
		public bool TryAddCharacters(uint[] unicodes, out uint[] missingUnicodes, bool includeFontFeatures = false)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (!this.m_CharacterLookupDictionary.ContainsKey(num2))
				{
					...
					TMP_Character tmp_Character = new TMP_Character(num2, num3);
					if (this.m_GlyphLookupDictionary.ContainsKey(num3))
					{
						tmp_Character.glyph = this.m_GlyphLookupDictionary[num3];
						tmp_Character.textAsset = this;
					...
					{
						if (this.m_GlyphsToAddLookup.Add(num3))
						{
		...
		internal void UpdateGlyphAdjustmentRecords()
		{
			...
			while (num2 < glyphPairAdjustmentRecords.Length && glyphPairAdjustmentRecords[num2].firstAdjustmentRecord.glyphIndex != 0U)
			{
				...
				if (!this.m_FontFeatureTable.m_GlyphPairAdjustmentRecordLookupDictionary.ContainsKey(num3))
				{
					...
					this.m_FontFeatureTable.m_GlyphPairAdjustmentRecordLookupDictionary.Add(num3, tmp_GlyphPairAdjustmentRecord);
				}
				num2++;
			}
		}
		...
		internal void ClearAtlasTextures(bool setAtlasSizeToZero = false)
		{
			...
			{
				Debug.LogWarning(string.Concat(new string[] { "Unable to reset font asset [", base.name, "]'s atlas texture. Please make the texture [", texture2D.name, "] readable." }), texture2D);
				return;
			}
		...
		internal void UpgradeFontAsset()
		{
			...
			for (int j = 0; j < this.m_glyphInfoList.Count; j++)
			{
				...
				glyph.atlasIndex = 0;
				this.m_GlyphTable.Add(glyph);
				TMP_Character tmp_Character = new TMP_Character((uint)tmp_Glyph.id, this, glyph);
				if (tmp_Glyph.id == 32)
				{
					flag = true;
				}
