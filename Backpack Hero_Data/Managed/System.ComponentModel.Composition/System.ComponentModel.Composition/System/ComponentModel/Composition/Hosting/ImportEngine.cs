	...
	public class ImportEngine : ICompositionService, IDisposable
	{
		...
		private CompositionResult TryPreviewImportsStateMachine(ImportEngine.PartManager partManager, ComposablePart part, AtomicComposition atomicComposition)
		{
			...
			if (partManager.State == ImportEngine.ImportState.NoImportsSatisfied)
			{
				...
				IEnumerable<ImportDefinition> enumerable = part.ImportDefinitions.Where(new Func<ImportDefinition, bool>(ImportEngine.IsRequiredImportForPreview));
		...
		private CompositionResult TrySatisfyImportsStateMachine(ImportEngine.PartManager partManager, ComposablePart part)
		{
			...
			while (partManager.State < ImportEngine.ImportState.Composed)
			{
				...
				case ImportEngine.ImportState.ImportsPreviewed:
				{
					...
					IEnumerable<ImportDefinition> enumerable = part.ImportDefinitions.Where((ImportDefinition import) => import.IsPrerequisite);
				...
				case ImportEngine.ImportState.PreExportImportsSatisfied:
				{
					...
					IEnumerable<ImportDefinition> enumerable2 = part.ImportDefinitions.Where((ImportDefinition import) => !import.IsPrerequisite);
		...
		private void OnExportsChanging(object sender, ExportsChangeEventArgs e)
		{
			...
			IEnumerable<ExportDefinition> enumerable2 = e.AddedExports.ConcatAllowingNull(e.RemovedExports);
		...
		private class PartManager
		{
			...
			public IEnumerable<string> GetImportedContractNames()
			{
				...
				{
					this._importedContractNames = this.Part.ImportDefinitions.Select((ImportDefinition import) => import.ContractName ?? ImportDefinition.EmptyContractName).Distinct<string>().ToArray<string>();
				}
			...
			public void DisposeAllDependencies()
			{
				if (this._importedDisposableExports != null)
				{
					IEnumerable<IDisposable> enumerable = this._importedDisposableExports.Values.SelectMany((List<IDisposable> exports) => exports);
					this._importedDisposableExports = null;
					enumerable.ForEach(delegate(IDisposable disposableExport)
					{
		...
		private class RecompositionManager
		{
			...
			{
				return part.ImportDefinitions.Where((ImportDefinition import) => ImportEngine.RecompositionManager.IsAffectedImport(import, changedExports));
			}
