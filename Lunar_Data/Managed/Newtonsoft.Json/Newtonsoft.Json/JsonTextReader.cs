		...
		private void ParseReadNumber(ReadType readType, char firstChar, int initialPosition)
		{
			...
			case ReadType.ReadAsInt64:
			{
				...
				if (flag2)
				{
					...
					try
					{
						obj = BoxedPrimitives.Get(text.StartsWith("0x", 5) ? Convert.ToInt64(text, 16) : Convert.ToInt64(text, 8));
					}
					catch (Exception ex)
					{
				...
				if (parseResult != ParseResult.Overflow)
				{
					...
					{
						double num3;
						if (!double.TryParse(this._stringReference.ToString(), 167, CultureInfo.InvariantCulture, ref num3))
						{
			...
			case ReadType.ReadAsInt32:
				...
				else
				{
					if (flag2)
					{
						...
						try
						{
							obj = BoxedPrimitives.Get(text3.StartsWith("0x", 5) ? Convert.ToInt32(text3, 16) : Convert.ToInt32(text3, 8));
							goto IL_027A;
						}
						catch (Exception ex2)
						{
			...
			case ReadType.ReadAsString:
			{
				...
				double num5;
				if (!double.TryParse(text4, 167, CultureInfo.InvariantCulture, ref num5))
				{
					throw this.ThrowReaderError("Input string '{0}' is not a valid number.".FormatWith(CultureInfo.InvariantCulture, this._stringReference.ToString()), null);
				}
				IL_0170:
			...
			case ReadType.ReadAsDecimal:
				...
				else
				{
					if (flag2)
					{
						...
						try
						{
							obj = BoxedPrimitives.Get(Convert.ToDecimal(text5.StartsWith("0x", 5) ? Convert.ToInt64(text5, 16) : Convert.ToInt64(text5, 8)));
							goto IL_035F;
						}
						catch (Exception ex4)
						{
			...
			case ReadType.ReadAsDouble:
				...
				else
				{
					if (flag2)
					{
						...
						try
						{
							obj = BoxedPrimitives.Get(Convert.ToDouble(text6.StartsWith("0x", 5) ? Convert.ToInt64(text6, 16) : Convert.ToInt64(text6, 8)));
							goto IL_0437;
						}
						catch (Exception ex5)
						{
