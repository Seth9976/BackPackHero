	...
	public class CCDSolver2D : Solver2D
	{
		...
		protected override void DoUpdateIK(List<Vector3> effectorPositions)
		{
			...
			if (CCD2D.Solve(vector, this.GetPlaneRootTransform().forward, this.iterations, this.tolerance, Mathf.Lerp(0.01f, 1f, this.m_Velocity), ref this.m_Positions))
			{
				...
				{
					Vector3 localPosition = this.m_Chain.transforms[i + 1].localPosition;
					Vector3 vector3 = this.m_Chain.transforms[i].InverseTransformPoint(this.m_Positions[i + 1]);
					this.m_Chain.transforms[i].localRotation *= Quaternion.FromToRotation(localPosition, vector3);
				}
			}
		}
