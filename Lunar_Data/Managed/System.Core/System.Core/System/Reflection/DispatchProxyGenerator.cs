	...
	internal static class DispatchProxyGenerator
	{
		...
		private class ProxyAssembly
		{
			...
			private TypeInfo GenerateTypeInfoOfIgnoresAccessChecksToAttribute()
			{
				...
				ConstructorInfo constructorInfo = typeInfo.DeclaredConstructors.Single((ConstructorInfo c) => c.GetParameters().Count<ParameterInfo>() == 1 && c.GetParameters()[0].ParameterType == typeof(AttributeTargets));
		...
		private class ProxyBuilder
		{
			...
			private void Complete()
			{
				...
				ConstructorInfo constructorInfo = this._proxyBaseType.GetTypeInfo().DeclaredConstructors.SingleOrDefault((ConstructorInfo c) => c.IsPublic && c.GetParameters().Length == 0);
				ilgenerator.Emit(OpCodes.Ldarg_0);
