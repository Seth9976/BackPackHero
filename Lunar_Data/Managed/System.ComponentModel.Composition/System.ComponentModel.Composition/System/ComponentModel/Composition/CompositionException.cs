	...
	public class CompositionException : Exception
	{
			...
			: base(message, innerException)
		{
			...
			base.SerializeObjectState += delegate(object exception, SafeSerializationEventArgs eventArgs)
			{
				...
				{
					compositionExceptionData._errors = this._errors.Select((CompositionError error) => new CompositionError(error.Id, error.Description, error.Element.ToSerializableElement(), error.Exception)).ToArray<CompositionError>();
				}
				else
				{
		...
		private static void WritePath(StringBuilder writer, IEnumerable<CompositionError> path, int ordinal)
		{
			...
			foreach (CompositionError compositionError in path.Skip(1))
			{
				writer.AppendLine();
