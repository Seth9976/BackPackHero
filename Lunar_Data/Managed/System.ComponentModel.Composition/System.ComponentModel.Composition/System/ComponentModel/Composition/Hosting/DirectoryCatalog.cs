	...
	public class DirectoryCatalog : ComposablePartCatalog, INotifyComposablePartCatalogChanged, ICompositionElement
	{
		...
		public void Refresh()
		{
			...
			for (;;)
			{
				...
				array2 = list.SelectMany((Tuple<string, AssemblyCatalog> cat) => cat.Item2).ToArray<ComposablePartDefinition>();
				array3 = list2.SelectMany((Tuple<string, AssemblyCatalog> cat) => cat.Item2).ToArray<ComposablePartDefinition>();
				using (AtomicComposition atomicComposition = new AtomicComposition())
				{
					ComposablePartCatalogChangeEventArgs composablePartCatalogChangeEventArgs = new ComposablePartCatalogChangeEventArgs(array2, array3, atomicComposition);
					this.OnChanging(composablePartCatalogChangeEventArgs);
					using (new WriteLock(this._thisLock))
					{
			...
			return;
			IL_01CF:
			ComposablePartCatalogChangeEventArgs composablePartCatalogChangeEventArgs2 = new ComposablePartCatalogChangeEventArgs(array2, array3, null);
			this.OnChanged(composablePartCatalogChangeEventArgs2);
		}
		...
		internal class DirectoryCatalogDebuggerProxy
		{
			...
			public ReadOnlyCollection<Assembly> Assemblies
			{
				get
				{
					return this._catalog._assemblyCatalogs.Values.Select((AssemblyCatalog catalog) => catalog.Assembly).ToReadOnlyCollection<Assembly>();
				}
			}
