...
namespace Unity.Jobs
{
	...
	public static class JobParallelIndexListExtensions
	{
		...
		public unsafe static JobHandle ScheduleAppend<T>(this T jobData, NativeList<int> indices, int arrayLength, int innerloopBatchCount, JobHandle dependsOn = default(JobHandle)) where T : struct, IJobParallelForFilter
		{
			...
			IntPtr intPtr = *JobParallelIndexListExtensions.JobParallelForFilterProducer<T>.jobReflectionData.Data;
			JobsUtility.JobScheduleParameters jobScheduleParameters = new JobsUtility.JobScheduleParameters(UnsafeUtility.AddressOf<JobParallelIndexListExtensions.JobParallelForFilterProducer<T>.JobWrapper>(ref jobWrapper), intPtr, dependsOn, ScheduleMode.Single);
		...
		public unsafe static void RunAppend<T>(this T jobData, NativeList<int> indices, int arrayLength) where T : struct, IJobParallelForFilter
		{
			...
			IntPtr intPtr = *JobParallelIndexListExtensions.JobParallelForFilterProducer<T>.jobReflectionData.Data;
			JobsUtility.JobScheduleParameters jobScheduleParameters = new JobsUtility.JobScheduleParameters(UnsafeUtility.AddressOf<JobParallelIndexListExtensions.JobParallelForFilterProducer<T>.JobWrapper>(ref jobWrapper), intPtr, default(JobHandle), ScheduleMode.Run);
