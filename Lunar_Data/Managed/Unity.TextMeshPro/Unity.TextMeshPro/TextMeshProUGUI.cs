	...
	public class TextMeshProUGUI : TMP_Text, ILayoutElement
	{
		...
		internal override int SetArraySizes(TMP_Text.UnicodeChar[] unicodeChars)
		{
			...
			while (num2 < unicodeChars.Length && unicodeChars[num2].unicode != 0)
			{
				...
				TMP_TextElement tmp_TextElement = base.GetTextElement((uint)num3, this.m_currentFontAsset, this.m_FontStyleInternal, this.m_FontWeightInternal, out flag);
				if (tmp_TextElement == null)
				{
					...
					tmp_TextElement = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)num3, this.m_currentFontAsset, true, this.m_FontStyleInternal, this.m_FontWeightInternal, out flag);
					if (tmp_TextElement == null && TMP_Settings.fallbackFontAssets != null && TMP_Settings.fallbackFontAssets.Count > 0)
					{
						tmp_TextElement = TMP_FontAssetUtilities.GetCharacterFromFontAssets((uint)num3, this.m_currentFontAsset, TMP_Settings.fallbackFontAssets, true, this.m_FontStyleInternal, this.m_FontWeightInternal, out flag);
					}
					if (tmp_TextElement == null && TMP_Settings.defaultFontAsset != null)
					{
						tmp_TextElement = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)num3, TMP_Settings.defaultFontAsset, true, this.m_FontStyleInternal, this.m_FontWeightInternal, out flag);
					}
					if (tmp_TextElement == null)
					{
		...
		protected virtual void GenerateTextMesh()
		{
			...
			while (num14 < this.m_TextProcessingArray.Length && this.m_TextProcessingArray[num14].unicode != 0)
			{
				...
				if (this.m_enableKerning)
				{
					...
					{
						uint num27 = (this.m_textInfo.characterInfo[this.m_characterCount + 1].textElement.m_GlyphIndex << 16) | glyphIndex;
						TMP_GlyphPairAdjustmentRecord tmp_GlyphPairAdjustmentRecord;
						if (this.m_currentFontAsset.m_FontFeatureTable.m_GlyphPairAdjustmentRecordLookupDictionary.TryGetValue(num27, out tmp_GlyphPairAdjustmentRecord))
						{
					...
					if (this.m_characterCount >= 1)
					{
						...
						uint num28 = (glyphIndex << 16) | glyphIndex2;
						TMP_GlyphPairAdjustmentRecord tmp_GlyphPairAdjustmentRecord;
						if (this.m_currentFontAsset.m_FontFeatureTable.m_GlyphPairAdjustmentRecordLookupDictionary.TryGetValue(num28, out tmp_GlyphPairAdjustmentRecord))
						{
				...
				{
					num29 = (this.m_monoSpacing / 2f - (metrics.width / 2f + metrics.horizontalBearingX) * num2) * (1f - this.m_charWidthAdjDelta);
				...
				if (this.m_textElementType == TMP_TextElementType.Character && !isUsingAlternateTypeface && (this.m_FontStyleInternal & FontStyles.Bold) == FontStyles.Bold)
				{
					if (this.m_currentMaterial != null && this.m_currentMaterial.HasProperty(ShaderUtilities.ID_GradientScale))
					{
						...
						num30 = this.m_currentFontAsset.boldStyle / 4f * @float * this.m_currentMaterial.GetFloat(ShaderUtilities.ID_ScaleRatio_A);
						if (num30 + num5 > @float)
						{
							num5 = @float - num30;
						}
					...
					num31 = this.m_currentFontAsset.boldSpacing;
				}
				else
				{
					if (this.m_currentMaterial != null && this.m_currentMaterial.HasProperty(ShaderUtilities.ID_GradientScale) && this.m_currentMaterial.HasProperty(ShaderUtilities.ID_ScaleRatio_A))
					{
						...
						num30 = this.m_currentFontAsset.normalStyle / 4f * float2 * this.m_currentMaterial.GetFloat(ShaderUtilities.ID_ScaleRatio_A);
						if (num30 + num5 > float2)
						{
							num5 = float2 - num30;
						}
					}
				...
				Vector3 vector3;
				vector3.x = this.m_xAdvance + (metrics.horizontalBearingX - num5 - num30 + tmp_GlyphValueRecord.m_XPlacement) * num2 * (1f - this.m_charWidthAdjDelta);
				...
				vector4.z = 0f;
				Vector3 vector5;
				vector5.x = vector4.x + (metrics.width + num5 * 2f + num30 * 2f) * num2 * (1f - this.m_charWidthAdjDelta);
				vector5.y = vector3.y;
				vector5.z = 0f;
				Vector3 vector6;
				vector6.x = vector5.x;
				...
				{
					float num32 = (float)this.m_ItalicAngle * 0.01f;
					Vector3 vector7 = new Vector3(num32 * ((metrics.horizontalBearingY + num5 + num30) * num2), 0f, 0f);
					Vector3 vector8 = new Vector3(num32 * ((metrics.horizontalBearingY - metrics.height - num5 - num30) * num2), 0f, 0f);
				...
				float num33 = ((this.m_textElementType == TMP_TextElementType.Character) ? (num18 * num2 / num16 + this.m_baselineOffset) : (num18 * num2 + this.m_baselineOffset));
				float num34 = ((this.m_textElementType == TMP_TextElementType.Character) ? (num19 * num2 / num16 + this.m_baselineOffset) : (num19 * num2 + this.m_baselineOffset));
				float num35 = num33;
				float num36 = num34;
				...
				if (flag11 || !flag10)
				{
					...
					{
						num35 = Mathf.Max((num33 - this.m_baselineOffset) / this.m_fontScaleMultiplier, num35);
						num36 = Mathf.Min((num34 - this.m_baselineOffset) / this.m_fontScaleMultiplier, num36);
					}
					this.m_maxLineAscender = Mathf.Max(num35, this.m_maxLineAscender);
					this.m_maxLineDescender = Mathf.Min(num36, this.m_maxLineDescender);
				}
				if (flag11 || !flag10)
				{
					this.m_textInfo.characterInfo[this.m_characterCount].adjustedAscender = num35;
					this.m_textInfo.characterInfo[this.m_characterCount].adjustedDescender = num36;
					this.m_ElementAscender = (this.m_textInfo.characterInfo[this.m_characterCount].ascender = num33 - this.m_lineOffset);
					this.m_ElementDescender = (this.m_textInfo.characterInfo[this.m_characterCount].descender = num34 - this.m_lineOffset);
				}
				else
				{
				...
				{
					this.m_PageAscender = ((this.m_PageAscender > num33) ? this.m_PageAscender : num33);
				}
				this.m_textInfo.characterInfo[this.m_characterCount].isVisible = false;
				...
				if (num4 == 9 || (!flag10 && num4 != 8203 && num4 != 173 && num4 != 3) || (num4 == 173 && !flag8) || this.m_textElementType == TMP_TextElementType.Sprite)
				{
					...
					float num37 = this.m_marginLeft;
					float num38 = this.m_marginRight;
					if (flag9)
					{
						num37 = this.m_textInfo.lineInfo[this.m_lineNumber].marginLeft;
						num38 = this.m_textInfo.lineInfo[this.m_lineNumber].marginRight;
					}
					num10 = ((this.m_width != -1f) ? Mathf.Min(num8 + 0.0001f - num37 - num38, this.m_width) : (num8 + 0.0001f - num37 - num38));
					float num39 = Mathf.Abs(this.m_xAdvance) + ((!this.m_isRightToLeft) ? metrics.horizontalAdvance : 0f) * (1f - this.m_charWidthAdjDelta) * ((num4 == 173) ? num25 : num2);
					float num40 = this.m_maxTextAscender - (this.m_maxLineDescender - this.m_lineOffset) + ((this.m_lineOffset > 0f && !this.m_IsDrivenLineSpacing) ? (this.m_maxLineAscender - this.m_startOfLineAscender) : 0f);
					...
					if (num40 > num9 + 0.0001f)
					{
						...
						if (this.m_enableAutoSizing)
						{
							...
							{
								float num41 = (num9 - num40) / (float)this.m_lineNumber;
								this.m_lineSpacingDelta = Mathf.Max(this.m_lineSpacingDelta + num41 / num, this.m_lineSpacingMax);
								return;
							}
							if (this.m_fontSize > this.m_fontSizeMin && this.m_AutoSizeIterationCount < this.m_AutoSizeMaxIterationCount)
							{
								...
								float num42 = Mathf.Max((this.m_fontSize - this.m_minFontSize) / 2f, 0.05f);
								this.m_fontSize -= num42;
					...
					if (num39 > num10 * (flag12 ? 1.05f : 1f))
					{
						if (this.m_enableWordWrapping && this.m_characterCount != this.m_firstCharacterOfLine)
						{
							...
							if (this.m_lineHeight == -32767f)
							{
								...
								num43 = ((this.m_lineOffset > 0f && !this.m_IsDrivenLineSpacing) ? (this.m_maxLineAscender - this.m_startOfLineAscender) : 0f) - this.m_maxLineDescender + adjustedAscender + (num6 + this.m_lineSpacingDelta) * num + this.m_lineSpacing * num3;
							}
							else
							{
								num43 = this.m_lineHeight + this.m_lineSpacing * num3;
								this.m_IsDrivenLineSpacing = true;
							}
							float num44 = this.m_maxTextAscender + num43 + this.m_lineOffset - this.m_textInfo.characterInfo[this.m_characterCount].adjustedDescender;
							if (this.m_textInfo.characterInfo[this.m_characterCount - 1].character == '\u00ad' && !flag8 && (this.m_overflowMode == TextOverflowModes.Overflow || num44 < num9 + 0.0001f))
							{
								characterSubstitution.index = this.m_characterCount - 1;
							...
							if (this.m_enableAutoSizing && flag6)
							{
								if (this.m_charWidthAdjDelta < this.m_charWidthMaxAdj / 100f && this.m_AutoSizeIterationCount < this.m_AutoSizeMaxIterationCount)
								{
									...
									float num46 = num39 - (num10 - 0.0001f) * (flag12 ? 1.05f : 1f);
							...
							if (num44 <= num9 + 0.0001f)
							{
								base.InsertNewLine(num14, num, num2, num3, this.m_GlyphHorizontalAdvanceAdjustment, num31, num26, num10, num6, ref flag5, ref num11);
								flag4 = true;
								flag6 = true;
								goto IL_3BD4;
							}
							...
							if (this.m_enableAutoSizing)
							{
								...
								{
									float num48 = (num9 - num44) / (float)(this.m_lineNumber + 1);
									this.m_lineSpacingDelta = Mathf.Max(this.m_lineSpacingDelta + num48 / num, this.m_lineSpacingMax);
									return;
								}
								if (this.m_charWidthAdjDelta < this.m_charWidthMaxAdj / 100f && this.m_AutoSizeIterationCount < this.m_AutoSizeMaxIterationCount)
								{
									...
									float num50 = num39 - (num10 - 0.0001f) * (flag12 ? 1.05f : 1f);
						...
						else
						{
							if (this.m_enableAutoSizing && this.m_AutoSizeIterationCount < this.m_AutoSizeMaxIterationCount)
							{
								if (this.m_charWidthAdjDelta < this.m_charWidthMaxAdj / 100f)
								{
									...
									if (this.m_charWidthAdjDelta > 0f)
									{
										num52 /= 1f - this.m_charWidthAdjDelta;
									}
									float num53 = num39 - (num10 - 0.0001f) * (flag12 ? 1.05f : 1f);
									this.m_charWidthAdjDelta += num53 / num52;
									this.m_charWidthAdjDelta = Mathf.Min(this.m_charWidthAdjDelta, this.m_charWidthMaxAdj / 100f);
									return;
								}
								...
								{
									this.m_maxFontSize = this.m_fontSize;
									float num54 = Mathf.Max((this.m_fontSize - this.m_minFontSize) / 2f, 0.05f);
									this.m_fontSize -= num54;
									this.m_fontSize = Mathf.Max((float)((int)(this.m_fontSize * 20f + 0.5f)) / 20f, this.m_fontSizeMin);
									return;
								}
							}
					...
					else
					{
						...
						if (this.m_textElementType == TMP_TextElementType.Character)
						{
							this.SaveGlyphVertexInfo(num5, num30, color);
						}
						...
						this.m_lastVisibleCharacterOfLine = this.m_characterCount;
						this.m_textInfo.lineInfo[this.m_lineNumber].marginLeft = num37;
						this.m_textInfo.lineInfo[this.m_lineNumber].marginRight = num38;
					}
				}
				else
				{
					...
					{
						float num55 = this.m_maxTextAscender - (this.m_maxLineDescender - this.m_lineOffset) + ((this.m_lineOffset > 0f && !this.m_IsDrivenLineSpacing) ? (this.m_maxLineAscender - this.m_startOfLineAscender) : 0f);
						int characterCount2 = this.m_characterCount;
						if (num55 > num9 + 0.0001f)
						{
							if (this.m_firstOverflowCharacterIndex == -1)
							{
								this.m_firstOverflowCharacterIndex = this.m_characterCount;
				...
				{
					float num56 = this.m_currentFontSize / (float)this.m_Ellipsis.fontAsset.m_FaceInfo.pointSize * this.m_Ellipsis.fontAsset.m_FaceInfo.scale * (this.m_isOrthographic ? 1f : 0.1f) * this.m_fontScaleMultiplier * this.m_Ellipsis.character.m_Scale * this.m_Ellipsis.character.m_Glyph.scale;
					float num57 = this.m_marginLeft;
					float num58 = this.m_marginRight;
					if (num4 == 10 && this.m_characterCount != this.m_firstCharacterOfLine)
					{
						num56 = this.m_textInfo.characterInfo[this.m_characterCount - 1].pointSize / (float)this.m_Ellipsis.fontAsset.m_FaceInfo.pointSize * this.m_Ellipsis.fontAsset.m_FaceInfo.scale * (this.m_isOrthographic ? 1f : 0.1f) * this.m_fontScaleMultiplier * this.m_Ellipsis.character.m_Scale * this.m_Ellipsis.character.m_Glyph.scale;
						num57 = this.m_textInfo.lineInfo[this.m_lineNumber].marginLeft;
						num58 = this.m_textInfo.lineInfo[this.m_lineNumber].marginRight;
					}
					...
					float num60 = Mathf.Abs(this.m_xAdvance) + ((!this.m_isRightToLeft) ? this.m_Ellipsis.character.m_Glyph.metrics.horizontalAdvance : 0f) * (1f - this.m_charWidthAdjDelta) * num56;
					float num61 = ((this.m_width != -1f) ? Mathf.Min(num8 + 0.0001f - num57 - num58, this.m_width) : (num8 + 0.0001f - num57 - num58));
					if (num60 < num61 * (flag12 ? 1.05f : 1f) && num59 < num9 + 0.0001f)
					{
				...
				if (num4 == 9)
				{
					float num62 = this.m_currentFontAsset.m_FaceInfo.tabWidth * (float)this.m_currentFontAsset.tabSize * num2;
					float num63 = Mathf.Ceil(this.m_xAdvance / num62) * num62;
					this.m_xAdvance = ((num63 > this.m_xAdvance) ? num63 : (this.m_xAdvance + num62));
				}
				else if (this.m_monoSpacing != 0f)
				{
					this.m_xAdvance += (this.m_monoSpacing - num29 + (this.m_currentFontAsset.normalSpacingOffset + num26) * num3 + this.m_cSpacing) * (1f - this.m_charWidthAdjDelta);
					if (flag10 || num4 == 8203)
					{
						this.m_xAdvance += this.m_wordSpacing * num3;
					}
				...
				{
					this.m_xAdvance -= (tmp_GlyphValueRecord.m_XAdvance * num2 + (this.m_currentFontAsset.normalSpacingOffset + num26 + num31) * num3 + this.m_cSpacing) * (1f - this.m_charWidthAdjDelta);
					if (flag10 || num4 == 8203)
					{
						this.m_xAdvance -= this.m_wordSpacing * num3;
					}
				...
				else
				{
					...
					if (this.m_isFXMatrixSet)
					{
						num64 = this.m_FXMatrix.lossyScale.x;
					}
					this.m_xAdvance += ((metrics.horizontalAdvance * num64 + tmp_GlyphValueRecord.m_XAdvance) * num2 + (this.m_currentFontAsset.normalSpacingOffset + num26 + num31) * num3 + this.m_cSpacing) * (1f - this.m_charWidthAdjDelta);
					if (flag10 || num4 == 8203)
					{
						this.m_xAdvance += this.m_wordSpacing * num3;
					}
				...
				{
					float num65 = this.m_maxLineAscender - this.m_startOfLineAscender;
					if (this.m_lineOffset > 0f && Math.Abs(num65) > 0.01f && !this.m_IsDrivenLineSpacing && !this.m_isNewPage)
					{
						base.AdjustLineOffset(this.m_firstCharacterOfLine, this.m_characterCount, num65);
						this.m_ElementDescender -= num65;
						this.m_lineOffset += num65;
						if (TMP_Text.m_SavedEllipsisState.lineNumber == this.m_lineNumber)
						{
							...
							TMP_Text.m_SavedEllipsisState.startOfLineAscender = TMP_Text.m_SavedEllipsisState.startOfLineAscender + num65;
							TMP_Text.m_SavedEllipsisState.lineOffset = TMP_Text.m_SavedEllipsisState.lineOffset + num65;
							TMP_Text.m_EllipsisInsertionCandidateStack.Push(TMP_Text.m_SavedEllipsisState);
						}
					}
					this.m_isNewPage = false;
					float num66 = this.m_maxLineAscender - this.m_lineOffset;
					float num67 = this.m_maxLineDescender - this.m_lineOffset;
					this.m_ElementDescender = ((this.m_ElementDescender < num67) ? this.m_ElementDescender : num67);
					if (!flag5)
					{
					...
					this.m_textInfo.lineInfo[this.m_lineNumber].lineExtents.min = new Vector2(this.m_textInfo.characterInfo[this.m_firstVisibleCharacterOfLine].bottomLeft.x, num67);
					this.m_textInfo.lineInfo[this.m_lineNumber].lineExtents.max = new Vector2(this.m_textInfo.characterInfo[this.m_lastVisibleCharacterOfLine].topRight.x, num66);
					...
					float num68 = ((this.m_currentFontAsset.normalSpacingOffset + num26 + num31) * num3 - this.m_cSpacing) * (1f - this.m_charWidthAdjDelta);
					if (this.m_textInfo.characterInfo[this.m_lastVisibleCharacterOfLine].isVisible)
					{
						this.m_textInfo.lineInfo[this.m_lineNumber].maxAdvance = this.m_textInfo.characterInfo[this.m_lastVisibleCharacterOfLine].xAdvance + (this.m_isRightToLeft ? num68 : (-num68));
					}
					else
					{
						this.m_textInfo.lineInfo[this.m_lineNumber].maxAdvance = this.m_textInfo.characterInfo[this.m_lastCharacterOfLine].xAdvance + (this.m_isRightToLeft ? num68 : (-num68));
					}
					this.m_textInfo.lineInfo[this.m_lineNumber].baseline = 0f - this.m_lineOffset;
					this.m_textInfo.lineInfo[this.m_lineNumber].ascender = num66;
					this.m_textInfo.lineInfo[this.m_lineNumber].descender = num67;
					this.m_textInfo.lineInfo[this.m_lineNumber].lineHeight = num66 - num67 + num6 * num;
					if (num4 == 10 || num4 == 11 || num4 == 45 || num4 == 8232 || num4 == 8233)
					{
						...
						if (this.m_lineHeight == -32767f)
						{
							float num69 = 0f - this.m_maxLineDescender + adjustedAscender2 + (num6 + this.m_lineSpacingDelta) * num + (this.m_lineSpacing + ((num4 == 10 || num4 == 8233) ? this.m_paragraphSpacing : 0f)) * num3;
			...
			float num70 = this.m_maxFontSize - this.m_minFontSize;
			if (this.m_enableAutoSizing && num70 > 0.051f && this.m_fontSize < this.m_fontSizeMax && this.m_AutoSizeIterationCount < this.m_AutoSizeMaxIterationCount)
			{
				if (this.m_charWidthAdjDelta < this.m_charWidthMaxAdj / 100f)
				{
					this.m_charWidthAdjDelta = 0f;
				...
				float num71 = Mathf.Max((this.m_maxFontSize - this.m_fontSize) / 2f, 0.05f);
				this.m_fontSize += num71;
			...
			int num72 = TMP_Text.m_materialReferences[this.m_Underline.materialIndex].referenceCount * 4;
			...
			if (verticalAlignment <= VerticalAlignmentOptions.Bottom)
			{
				if (verticalAlignment != VerticalAlignmentOptions.Top)
				{
					if (verticalAlignment != VerticalAlignmentOptions.Middle)
					{
						if (verticalAlignment == VerticalAlignmentOptions.Bottom)
						{
							...
							{
								vector11 = rectTransformCorners[0] + new Vector3(0f + margin.x, 0f - num11 + margin.w, 0f);
							}
							else
							{
					...
					{
						vector11 = (rectTransformCorners[0] + rectTransformCorners[1]) / 2f + new Vector3(0f + margin.x, 0f - (this.m_maxTextAscender + margin.y + num11 - margin.w) / 2f, 0f);
					}
					else
					{
				...
				{
					vector11 = rectTransformCorners[1] + new Vector3(0f + margin.x, 0f - this.m_maxTextAscender - margin.y, 0f);
				}
				else
				{
			...
			float num79 = (this.m_previousLossyScaleY = base.transform.lossyScale.y);
			...
			while (i < this.m_characterCount)
			{
				...
				if (isVisible)
				{
					...
					else
					{
						...
						float num89 = this.m_uvLineOffset * (float)lineNumber4 % 1f;
						...
						case TextureMappingOptions.Line:
							...
							{
								characterInfo[i].vertex_BL.uv2.x = (characterInfo[i].vertex_BL.position.x - lineExtents.min.x) / (lineExtents.max.x - lineExtents.min.x) + num89;
								characterInfo[i].vertex_TL.uv2.x = (characterInfo[i].vertex_TL.position.x - lineExtents.min.x) / (lineExtents.max.x - lineExtents.min.x) + num89;
								characterInfo[i].vertex_TR.uv2.x = (characterInfo[i].vertex_TR.position.x - lineExtents.min.x) / (lineExtents.max.x - lineExtents.min.x) + num89;
								characterInfo[i].vertex_BR.uv2.x = (characterInfo[i].vertex_BR.position.x - lineExtents.min.x) / (lineExtents.max.x - lineExtents.min.x) + num89;
							}
							else
							{
								characterInfo[i].vertex_BL.uv2.x = (characterInfo[i].vertex_BL.position.x + vector12.x - this.m_meshExtents.min.x) / (this.m_meshExtents.max.x - this.m_meshExtents.min.x) + num89;
								characterInfo[i].vertex_TL.uv2.x = (characterInfo[i].vertex_TL.position.x + vector12.x - this.m_meshExtents.min.x) / (this.m_meshExtents.max.x - this.m_meshExtents.min.x) + num89;
								characterInfo[i].vertex_TR.uv2.x = (characterInfo[i].vertex_TR.position.x + vector12.x - this.m_meshExtents.min.x) / (this.m_meshExtents.max.x - this.m_meshExtents.min.x) + num89;
								characterInfo[i].vertex_BR.uv2.x = (characterInfo[i].vertex_BR.position.x + vector12.x - this.m_meshExtents.min.x) / (this.m_meshExtents.max.x - this.m_meshExtents.min.x) + num89;
							}
							break;
						...
						case TextureMappingOptions.MatchAspect:
						{
							...
							case TextureMappingOptions.Character:
								...
								break;
							case TextureMappingOptions.Line:
								characterInfo[i].vertex_BL.uv2.y = (characterInfo[i].vertex_BL.position.y - lineExtents.min.y) / (lineExtents.max.y - lineExtents.min.y) + num89;
								characterInfo[i].vertex_TL.uv2.y = (characterInfo[i].vertex_TL.position.y - lineExtents.min.y) / (lineExtents.max.y - lineExtents.min.y) + num89;
								...
								break;
							case TextureMappingOptions.Paragraph:
								characterInfo[i].vertex_BL.uv2.y = (characterInfo[i].vertex_BL.position.y - this.m_meshExtents.min.y) / (this.m_meshExtents.max.y - this.m_meshExtents.min.y) + num89;
								characterInfo[i].vertex_TL.uv2.y = (characterInfo[i].vertex_TL.position.y - this.m_meshExtents.min.y) / (this.m_meshExtents.max.y - this.m_meshExtents.min.y) + num89;
								characterInfo[i].vertex_TR.uv2.y = characterInfo[i].vertex_BL.uv2.y;
								characterInfo[i].vertex_BR.uv2.y = characterInfo[i].vertex_TL.uv2.y;
								break;
							case TextureMappingOptions.MatchAspect:
							...
							float num90 = (1f - (characterInfo[i].vertex_BL.uv2.y + characterInfo[i].vertex_TL.uv2.y) * characterInfo[i].aspectRatio) / 2f;
							characterInfo[i].vertex_BL.uv2.x = characterInfo[i].vertex_BL.uv2.y * characterInfo[i].aspectRatio + num90 + num89;
							characterInfo[i].vertex_TL.uv2.x = characterInfo[i].vertex_BL.uv2.x;
							characterInfo[i].vertex_TR.uv2.x = characterInfo[i].vertex_TL.uv2.y * characterInfo[i].aspectRatio + num90 + num89;
						...
						case TextureMappingOptions.MatchAspect:
						{
							float num91 = (1f - (characterInfo[i].vertex_BL.uv2.x + characterInfo[i].vertex_TR.uv2.x) / characterInfo[i].aspectRatio) / 2f;
							characterInfo[i].vertex_BL.uv2.y = num91 + characterInfo[i].vertex_BL.uv2.x / characterInfo[i].aspectRatio;
							characterInfo[i].vertex_TL.uv2.y = num91 + characterInfo[i].vertex_TR.uv2.x / characterInfo[i].aspectRatio;
							characterInfo[i].vertex_BR.uv2.y = characterInfo[i].vertex_BL.uv2.y;
							characterInfo[i].vertex_TR.uv2.y = characterInfo[i].vertex_TL.uv2.y;
							break;
						}
						...
						case RenderMode.ScreenSpaceOverlay:
							num80 *= Mathf.Abs(num79) / scaleFactor;
							break;
						case RenderMode.ScreenSpaceCamera:
							num80 *= (flag17 ? Mathf.Abs(num79) : 1f);
							break;
						case RenderMode.WorldSpace:
							num80 *= Mathf.Abs(num79);
							break;
						}
						float num92 = characterInfo[i].vertex_BL.uv2.x;
						float num93 = characterInfo[i].vertex_BL.uv2.y;
						float num94 = characterInfo[i].vertex_TR.uv2.x;
						float num95 = characterInfo[i].vertex_TR.uv2.y;
						float num96 = (float)((int)num92);
						float num97 = (float)((int)num93);
						num92 -= num96;
						num94 -= num96;
						num93 -= num97;
						num95 -= num97;
						characterInfo[i].vertex_BL.uv2.x = base.PackUV(num92, num93);
						characterInfo[i].vertex_BL.uv2.y = num80;
						characterInfo[i].vertex_TL.uv2.x = base.PackUV(num92, num95);
						characterInfo[i].vertex_TL.uv2.y = num80;
						characterInfo[i].vertex_TR.uv2.x = base.PackUV(num94, num95);
						characterInfo[i].vertex_TR.uv2.y = num80;
						characterInfo[i].vertex_BR.uv2.x = base.PackUV(num94, num93);
						characterInfo[i].vertex_BR.uv2.y = num80;
					}
					if (i < this.m_maxVisibleCharacters && num75 < this.m_maxVisibleWords && lineNumber4 < this.m_maxVisibleLines && this.m_overflowMode != TextOverflowModes.Page)
					{
						TMP_CharacterInfo[] array = characterInfo;
						int num98 = i;
						array[num98].vertex_BL.position = array[num98].vertex_BL.position + vector13;
						TMP_CharacterInfo[] array2 = characterInfo;
						int num99 = i;
						array2[num99].vertex_TL.position = array2[num99].vertex_TL.position + vector13;
						TMP_CharacterInfo[] array3 = characterInfo;
						int num100 = i;
						array3[num100].vertex_TR.position = array3[num100].vertex_TR.position + vector13;
						TMP_CharacterInfo[] array4 = characterInfo;
						int num101 = i;
						array4[num101].vertex_BR.position = array4[num101].vertex_BR.position + vector13;
					}
					else if (i < this.m_maxVisibleCharacters && num75 < this.m_maxVisibleWords && lineNumber4 < this.m_maxVisibleLines && this.m_overflowMode == TextOverflowModes.Page && characterInfo[i].pageNumber == num7)
					{
						TMP_CharacterInfo[] array5 = characterInfo;
						int num102 = i;
						array5[num102].vertex_BL.position = array5[num102].vertex_BL.position + vector13;
						TMP_CharacterInfo[] array6 = characterInfo;
						int num103 = i;
						array6[num103].vertex_TL.position = array6[num103].vertex_TL.position + vector13;
						TMP_CharacterInfo[] array7 = characterInfo;
						int num104 = i;
						array7[num104].vertex_TR.position = array7[num104].vertex_TR.position + vector13;
						TMP_CharacterInfo[] array8 = characterInfo;
						int num105 = i;
						array8[num105].vertex_BR.position = array8[num105].vertex_BR.position + vector13;
					}
					else
					{
					...
					if (elementType == TMP_TextElementType.Character)
					{
						this.FillCharacterVertexBuffers(i, num73);
					}
					else if (elementType == TMP_TextElementType.Sprite)
					{
						this.FillSpriteVertexBuffers(i, num74);
					}
				}
				TMP_CharacterInfo[] characterInfo2 = this.m_textInfo.characterInfo;
				int num106 = i;
				characterInfo2[num106].bottomLeft = characterInfo2[num106].bottomLeft + vector13;
				TMP_CharacterInfo[] characterInfo3 = this.m_textInfo.characterInfo;
				int num107 = i;
				characterInfo3[num107].topLeft = characterInfo3[num107].topLeft + vector13;
				TMP_CharacterInfo[] characterInfo4 = this.m_textInfo.characterInfo;
				int num108 = i;
				characterInfo4[num108].topRight = characterInfo4[num108].topRight + vector13;
				TMP_CharacterInfo[] characterInfo5 = this.m_textInfo.characterInfo;
				int num109 = i;
				characterInfo5[num109].bottomRight = characterInfo5[num109].bottomRight + vector13;
				TMP_CharacterInfo[] characterInfo6 = this.m_textInfo.characterInfo;
				int num110 = i;
				characterInfo6[num110].origin = characterInfo6[num110].origin + vector13.x;
				TMP_CharacterInfo[] characterInfo7 = this.m_textInfo.characterInfo;
				int num111 = i;
				characterInfo7[num111].xAdvance = characterInfo7[num111].xAdvance + vector13.x;
				TMP_CharacterInfo[] characterInfo8 = this.m_textInfo.characterInfo;
				int num112 = i;
				characterInfo8[num112].ascender = characterInfo8[num112].ascender + vector13.y;
				TMP_CharacterInfo[] characterInfo9 = this.m_textInfo.characterInfo;
				int num113 = i;
				characterInfo9[num113].descender = characterInfo9[num113].descender + vector13.y;
				TMP_CharacterInfo[] characterInfo10 = this.m_textInfo.characterInfo;
				int num114 = i;
				characterInfo10[num114].baseLine = characterInfo10[num114].baseLine + vector13.y;
				if (lineNumber4 != num77 || i == this.m_characterCount - 1)
				{
					if (lineNumber4 != num77)
					{
						TMP_LineInfo[] lineInfo4 = this.m_textInfo.lineInfo;
						int num115 = num77;
						lineInfo4[num115].baseline = lineInfo4[num115].baseline + vector13.y;
						TMP_LineInfo[] lineInfo5 = this.m_textInfo.lineInfo;
						int num116 = num77;
						lineInfo5[num116].ascender = lineInfo5[num116].ascender + vector13.y;
						TMP_LineInfo[] lineInfo6 = this.m_textInfo.lineInfo;
						int num117 = num77;
						lineInfo6[num117].descender = lineInfo6[num117].descender + vector13.y;
						TMP_LineInfo[] lineInfo7 = this.m_textInfo.lineInfo;
						int num118 = num77;
						lineInfo7[num118].maxAdvance = lineInfo7[num118].maxAdvance + vector13.x;
						this.m_textInfo.lineInfo[num77].lineExtents.min = new Vector2(this.m_textInfo.characterInfo[this.m_textInfo.lineInfo[num77].firstCharacterIndex].bottomLeft.x, this.m_textInfo.lineInfo[num77].descender);
						this.m_textInfo.lineInfo[num77].lineExtents.max = new Vector2(this.m_textInfo.characterInfo[this.m_textInfo.lineInfo[num77].lastVisibleCharacterIndex].topRight.x, this.m_textInfo.lineInfo[num77].ascender);
					}
					if (i == this.m_characterCount - 1)
					{
						TMP_LineInfo[] lineInfo8 = this.m_textInfo.lineInfo;
						int num119 = lineNumber4;
						lineInfo8[num119].baseline = lineInfo8[num119].baseline + vector13.y;
						TMP_LineInfo[] lineInfo9 = this.m_textInfo.lineInfo;
						int num120 = lineNumber4;
						lineInfo9[num120].ascender = lineInfo9[num120].ascender + vector13.y;
						TMP_LineInfo[] lineInfo10 = this.m_textInfo.lineInfo;
						int num121 = lineNumber4;
						lineInfo10[num121].descender = lineInfo10[num121].descender + vector13.y;
						TMP_LineInfo[] lineInfo11 = this.m_textInfo.lineInfo;
						int num122 = lineNumber4;
						lineInfo11[num122].maxAdvance = lineInfo11[num122].maxAdvance + vector13.x;
				...
				if (char.IsLetterOrDigit(character) || character == '-' || character == '\u00ad' || character == '‐' || character == '‑')
				{
					...
					{
						int num123 = this.m_textInfo.wordInfo.Length;
						int wordCount = this.m_textInfo.wordCount;
						if (this.m_textInfo.wordCount + 1 > num123)
						{
							TMP_TextInfo.Resize<TMP_WordInfo>(ref this.m_textInfo.wordInfo, num123 + 1);
						}
						int num124 = i;
						this.m_textInfo.wordInfo[wordCount].firstCharacterIndex = num78;
						this.m_textInfo.wordInfo[wordCount].lastCharacterIndex = num124;
						this.m_textInfo.wordInfo[wordCount].characterCount = num124 - num78 + 1;
						...
						lineInfo12[num125].wordCount = lineInfo12[num125].wordCount + 1;
					}
				}
				...
				{
					int num124 = ((i == this.m_characterCount - 1 && char.IsLetterOrDigit(character)) ? i : (i - 1));
					flag16 = false;
					int num126 = this.m_textInfo.wordInfo.Length;
					int wordCount2 = this.m_textInfo.wordCount;
					if (this.m_textInfo.wordCount + 1 > num126)
					{
						TMP_TextInfo.Resize<TMP_WordInfo>(ref this.m_textInfo.wordInfo, num126 + 1);
					}
					this.m_textInfo.wordInfo[wordCount2].firstCharacterIndex = num78;
					this.m_textInfo.wordInfo[wordCount2].lastCharacterIndex = num124;
					this.m_textInfo.wordInfo[wordCount2].characterCount = num124 - num78 + 1;
					this.m_textInfo.wordInfo[wordCount2].textComponent = this;
					num75++;
					this.m_textInfo.wordCount++;
					TMP_LineInfo[] lineInfo13 = this.m_textInfo.lineInfo;
					int num127 = lineNumber4;
					lineInfo13[num127].wordCount = lineInfo13[num127].wordCount + 1;
				}
				if ((this.m_textInfo.characterInfo[i].style & FontStyles.Underline) == FontStyles.Underline)
				{
					...
					this.m_textInfo.characterInfo[i].underlineVertexIndex = num72;
					...
					if (flag && this.m_characterCount == 1)
					{
						...
						float num128 = this.m_textInfo.characterInfo[i].scale;
						this.DrawUnderlineMesh(zero, zero2, ref num72, num82, num128, num83, num81, color2);
						num83 = 0f;
						num81 = 0f;
					...
					else if (flag && (i == tmp_LineInfo.lastCharacterIndex || i >= tmp_LineInfo.lastVisibleCharacterIndex))
					{
						...
						if (char.IsWhiteSpace(character) || character == '\u200b')
						{
							...
							num128 = this.m_textInfo.characterInfo[lastVisibleCharacterIndex].scale;
						}
						else
						{
							zero2 = new Vector3(this.m_textInfo.characterInfo[i].topRight.x, num84, 0f);
							num128 = this.m_textInfo.characterInfo[i].scale;
						}
						flag = false;
						this.DrawUnderlineMesh(zero, zero2, ref num72, num82, num128, num83, num81, color2);
						num83 = 0f;
					...
					else if (flag && !flag18)
					{
						...
						float num128 = this.m_textInfo.characterInfo[i - 1].scale;
						this.DrawUnderlineMesh(zero, zero2, ref num72, num82, num128, num83, num81, color2);
						num83 = 0f;
						num81 = 0f;
						num84 = TMP_Text.k_LargePositiveFloat;
				...
				{
					bool flag20 = true;
					this.m_textInfo.characterInfo[i].strikethroughVertexIndex = num72;
					...
					if (!flag2 && flag20 && i <= tmp_LineInfo.lastVisibleCharacterIndex && character != '\n' && character != '\v' && character != '\r' && (i != tmp_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character)))
					{
						...
						num88 = this.m_textInfo.characterInfo[i].baseLine;
					}
					if (flag2 && this.m_characterCount == 1)
					{
						...
						this.DrawUnderlineMesh(zero3, zero4, ref num72, num87, num87, num87, num80, color3);
					}
					...
					else if (flag2 && i < this.m_characterCount && (this.m_textInfo.characterInfo[i + 1].pointSize != num86 || !TMP_Math.Approximately(this.m_textInfo.characterInfo[i + 1].baseLine + vector13.y, num88)))
					{
						flag2 = false;
				...
				if ((this.m_textInfo.characterInfo[i].style & FontStyles.Highlight) == FontStyles.Highlight)
				{
					...
					if (flag3)
					{
						...
						if (highlightState != tmp_CharacterInfo.highlightState)
						{
							...
							this.DrawTextHighlight(vector, vector2, ref num72, highlightState.color);
							flag3 = true;
				...
				{
					if (lineNumber4 != num77 || i == 0 || i == this.m_firstVisibleCharacter)
					{
						if (!this.m_isRightToLeft)
						{
					...
					float num129 = ((!this.m_isRightToLeft) ? (tmp_LineInfo.width - tmp_LineInfo.maxAdvance) : (tmp_LineInfo.width + tmp_LineInfo.maxAdvance));
					int num130 = tmp_LineInfo.visibleCharacterCount - 1 + tmp_LineInfo.controlCharacterCount;
					int num131 = (characterInfo[tmp_LineInfo.lastCharacterIndex].isVisible ? tmp_LineInfo.spaceCount : (tmp_LineInfo.spaceCount - 1)) - tmp_LineInfo.controlCharacterCount;
					if (flag15)
					{
						num131--;
						num130++;
					}
					float num132 = ((num131 > 0) ? this.m_wordWrappingRatios : 1f);
					if (num131 < 1)
					{
						num131 = 1;
					}
					if (character != '\u00a0' && (character == '\t' || char.IsSeparator(character)))
					{
						if (!this.m_isRightToLeft)
						{
							vector12 += new Vector3(num129 * (1f - num132) / (float)num131, 0f, 0f);
							goto IL_467C;
						}
						vector12 -= new Vector3(num129 * (1f - num132) / (float)num131, 0f, 0f);
						goto IL_467C;
					...
					else
					{
						if (!this.m_isRightToLeft)
						{
							vector12 += new Vector3(num129 * num132 / (float)num130, 0f, 0f);
							goto IL_467C;
						}
						vector12 -= new Vector3(num129 * num132 / (float)num130, 0f, 0f);
						goto IL_467C;
			...
			this.m_textInfo.lineCount = num76;
			this.m_textInfo.wordCount = ((num75 != 0 && this.m_characterCount > 0) ? num75 : 1);
