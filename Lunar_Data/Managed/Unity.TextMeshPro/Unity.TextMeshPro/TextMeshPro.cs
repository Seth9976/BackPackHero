	...
	public class TextMeshPro : TMP_Text, ILayoutElement
	{
		...
		internal override int SetArraySizes(TMP_Text.UnicodeChar[] unicodeChars)
		{
			...
			while (num2 < unicodeChars.Length && unicodeChars[num2].unicode != 0)
			{
				...
				bool flag2;
				TMP_TextElement tmp_TextElement = base.GetTextElement((uint)num3, this.m_currentFontAsset, this.m_FontStyleInternal, this.m_FontWeightInternal, out flag2);
				if (tmp_TextElement == null)
				{
					...
					tmp_TextElement = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)num3, this.m_currentFontAsset, true, this.m_FontStyleInternal, this.m_FontWeightInternal, out flag2);
					if (tmp_TextElement == null && TMP_Settings.fallbackFontAssets != null && TMP_Settings.fallbackFontAssets.Count > 0)
					{
						tmp_TextElement = TMP_FontAssetUtilities.GetCharacterFromFontAssets((uint)num3, this.m_currentFontAsset, TMP_Settings.fallbackFontAssets, true, this.m_FontStyleInternal, this.m_FontWeightInternal, out flag2);
					}
					if (tmp_TextElement == null && TMP_Settings.defaultFontAsset != null)
					{
						tmp_TextElement = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)num3, TMP_Settings.defaultFontAsset, true, this.m_FontStyleInternal, this.m_FontWeightInternal, out flag2);
					}
					if (tmp_TextElement == null)
					{
		...
		protected virtual void GenerateTextMesh()
		{
			...
			bool flag16 = false;
			int num78 = 0;
			float num79 = (this.m_previousLossyScaleY = this.transform.lossyScale.y);
			...
			while (i < this.m_characterCount)
			{
				...
				if (isVisible)
				{
					...
					else
					{
						...
						num80 = characterInfo[i].scale * Mathf.Abs(num79) * (1f - this.m_charWidthAdjDelta);
